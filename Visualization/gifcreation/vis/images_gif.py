...
# 
def images_gif(gdf, dates, map, df_list):
    """
        generate images for the gif, the images are showing intensities of bikes on each counters
        for each day in the data

        Entry : gdf = GeoDataFrame with counter data
                dates = list of dates
                map = map we want to show (here the city Montpellier)
                df_list = list of  counters dataframes
    """
    numFile=0
    for date in dates:
        print(date)
        ax = map.plot(figsize = (10, 10), zorder=1, edgecolor='gray', facecolor='none')
        ...
        ctx.add_basemap(ax, crs=map.crs.to_string(), source=ctx.providers.OpenStreetMap.Mapnik)
        ax.set_axis_off()
